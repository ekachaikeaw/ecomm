...
func TestCreateProduct(t *testing.T) {
	...
	tcs := []struct {
		...
		},
		{
			...
			test: func(t *testing.T, st *MySQLStorer, mock sqlmock.Sqlmock) {
				mock.ExpectExec("INSERT INTO products (name, image, category, description, rating, num_reviews, price, count_in_stock) VALUES (?, ?, ?, ?, ?, ?, ?, ?)").WillReturnError(fmt.Errorf("error creating product"))
		...
		{
			name: "failed getting last inserting ID",
			test: func(t *testing.T, st *MySQLStorer, mock sqlmock.Sqlmock) {
...
func TestGetOrder(t *testing.T) {
	...
	tcs := []struct{
	...
	} {
		{
			...
			test: func(t *testing.T, st *MySQLStorer, mock sqlmock.Sqlmock) {
				...
				oirows := sqlmock.NewRows([]string {"id", "name", "quantity", "image", "price", "product_id", "order_id"}).
					...
					AddRow(1, ois[1].Name, ois[1].Quantity, ois[1].Image, ois[1].Price, ois[1].ProductID, ois[1].OrderID)
		...
		},
		{
			...
			test: func(t *testing.T, st *MySQLStorer, mock sqlmock.Sqlmock) {
				mock.ExpectQuery("SELECT * FROM orders WHERE id=?").WithArgs(1).WillReturnError(fmt.Errorf("error getting orders"))
		...
		},
		{
			...
			test: func(t *testing.T, st *MySQLStorer, mock sqlmock.Sqlmock) {
				orows := sqlmock.NewRows([]string {"id", "payment_method", "shipping_price", "total_price", "created_at", "updated_at"}).
					AddRow(1, o.PaymentMethod, o.ShippingPrice, o.TotalPrice, o.CreatedAt, o.UpdatedAt)
	...
	tcs := []struct {
		...
		},
		{
			...
			test: func(t *testing.T, st *MySQLStorer, mock sqlmock.Sqlmock) {
				mock.ExpectBegin()
				mock.ExpectExec("DELETE FROM order_items WHERE order_id=?").WithArgs(1).WillReturnResult(sqlmock.NewResult(1, 1))
				mock.ExpectExec("DELETE FROM orders WHERE id=?").WithArgs(1).WillReturnError(fmt.Errorf("error deleting orders"))
				mock.ExpectRollback()
